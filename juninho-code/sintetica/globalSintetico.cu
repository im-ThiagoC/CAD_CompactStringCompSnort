...
const int sizeText = COLUNAS/LINHAS;
const long repeticoes9 = 1000; //mudar o nome para textos
...
void preencheEntrada9()
{
	...
	for(int k = 0; k < repeticoes9; k++)
	{
		for(int i = 0; i < pkts; i++, contAux++)
		{
			...
			{
				h_texto9[contAux][j] = pkt[0][j+(i*sizeText)];
			}
		}
	}
}
...
{ 
  ...
  //for(i=(idx*sizeText); d_texto9[i] != -1 ; i++) //percorre o texto
...
cudaProfilerStart();
	...
	blockSize=100;gridSize=pkts*repeticoes9/100;
